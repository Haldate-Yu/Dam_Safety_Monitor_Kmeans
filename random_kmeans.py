...
OUTPUT_DIR = "random_kmeans_results"  # 原始方法结果保存目录
...
def load_data(file_path):
    ...
    for sheet_name in xls.sheet_names:
        try:
            ...
            print(f"[随机KMeans] 成功读取测点 {sheet_name}，有效数据点数：{len(df)}")
        except Exception as e:
            print(f"[随机KMeans] 读取测点 {sheet_name} 出错：{str(e)}，已跳过")
    return sheet_data
...
def extract_features(sheet_data):
    ...
    if len(point_names) == 0:
        raise ValueError("[随机KMeans] 无有效测点数据")
...
# 3. 随机KMeans聚类（sklearn原生实现）
def random_kmeans(features_df, n_clusters=3):
    ...
    # 保存聚类结果
    result_path = os.path.join(OUTPUT_DIR, "随机KMeans聚类结果.xlsx")
    features_df.to_excel(result_path, sheet_name='聚类结果')
    print(f"[随机KMeans] 聚类结果已保存至：{result_path}")
...
def compute_coupling(features_df, sheet_data):
    ...
    coupling_path = os.path.join(OUTPUT_DIR, "随机KMeans耦合特征.xlsx")
    coupling_df.to_excel(coupling_path, index=False)
    print(f"[随机KMeans] 耦合特征已保存至：{coupling_path}")
    return coupling_df
...
def visualize(features_df):
    ...
    plt.title('随机KMeans聚类结果（均值-标准差散点图）', fontsize=18, fontweight='bold')
    ...
    plt.tight_layout()
    img_path = os.path.join(OUTPUT_DIR, "随机KMeans聚类可视化.png")
    plt.savefig(img_path, dpi=600, bbox_inches='tight')
    plt.close()
    print(f"[随机KMeans] 可视化图已保存至：{img_path}")
...
def main(file_path, n_clusters=3):
    try:
        print("=" * 50)
        print("开始执行随机KMeans聚类")
        print("=" * 50)
        # 1. 数据加载与特征提取
        sheet_data = load_data(file_path)
        features_df = extract_features(sheet_data)
        # 2. 聚类与结果输出
        result_df, _ = random_kmeans(features_df, n_clusters)
        # 3. 耦合特征计算
        compute_coupling(result_df, sheet_data)
        # 4. 可视化
        visualize(result_df)
        print("=" * 50)
        print("随机KMeans聚类执行完成")
        print("=" * 50)
        return result_df
    except Exception as e:
        print(f"[随机KMeans] 执行出错：{str(e)}")
